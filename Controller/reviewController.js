  ...
  exports.UpdateReview = async (req, res) => {
    try {
      ...
      const { images, ...updateFields } = req.body;
        const updatedReview = await Review.findByIdAndUpdate(id, { $set: updateFields }, { new: true });
      // Handle new image uploads (if any)
      if (req.files && req.files["images"]) {
        const newImages = req.files["images"].map((file) => `Uploads/${file.filename}`);
  
        // Append new images without removing existing ones
        await Review.findByIdAndUpdate(id, { $push: { images: { $each: newImages } } }, { new: true });
      }
  
      // Fetch and return the updated product
      const finalReview = await Review.findById(id);
      res.status(200).json({ status: true, message: "Review Updated Successfully", data: finalReview });
    } catch (err) {
